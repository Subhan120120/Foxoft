         ...
         using (subContext db = new subContext())
         {
            List<DcProduct> products = db.DcProducts.Include(x => x.TrInvoiceLines)
                                                    ...
                                                    .Select(x => new DcProduct
                                                    {
                                                       ...
                                                       LastPurchasePrice = x.TrInvoiceLines.Where(l => l.TrInvoiceHeader.ProcessCode == "RP").OrderByDescending(l => l.TrInvoiceHeader.DocumentDate).ThenByDescending(l => l.TrInvoiceHeader.DocumentDate).Select(x => x.PriceLoc - (x.PriceLoc * x.PosDiscount / 100)).FirstOrDefault(),
